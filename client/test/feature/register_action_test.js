...
casper.test.begin('Register/Unregister action test', function(test) {
  ...
  casper.waitForSelector(".ui-dialog-buttonset > button",
    function success() {
      test.assertExists(".ui-dialog-buttonset > button",
                        "Confirmation dialog button appeared when registering target action.");
      this.evaluate(function() {
        $("#server-selector").find("table#selectorMainTable").find("tr").last().click().change();
        $("div.ui-dialog-buttonset").attr("area-described-by","server-selector").last().find("button").click().change();
      });
    },
    function fail() {
      test.assertExists(".ui-dialog-buttonset > button");
  ...
  casper.waitForSelector(".ui-dialog-buttonset > button",
    function success() {
      test.assertExists(".ui-dialog-buttonset > button",
                        "Confirmation dialog button appeared when registering action.");
