                                          ...
                                          const OperationPrivilege &privilege)
{
	...
	if (getNullFlag(actionDef, ACTCOND_SERVER_ID) == ITEM_DATA_NOT_NULL)
		arg.add(IDX_ACTIONS_SERVER_ID, actionDef.condition.serverId);
	if (getNullFlag(actionDef, ACTCOND_HOST_ID) == ITEM_DATA_NOT_NULL)
		arg.add(IDX_ACTIONS_HOST_ID,  actionDef.condition.hostId);
	if (getNullFlag(actionDef, ACTCOND_HOST_GROUP_ID) == ITEM_DATA_NOT_NULL)
		arg.add(IDX_ACTIONS_HOST_GROUP_ID, actionDef.condition.hostgroupId);
	if (getNullFlag(actionDef, ACTCOND_TRIGGER_ID) == ITEM_DATA_NOT_NULL)
		arg.add(IDX_ACTIONS_TRIGGER_ID, actionDef.condition.triggerId);
	if (getNullFlag(actionDef, ACTCOND_TRIGGER_STATUS) == ITEM_DATA_NOT_NULL)
		arg.add(IDX_ACTIONS_TRIGGER_STATUS, actionDef.condition.triggerStatus);
	if (getNullFlag(actionDef, ACTCOND_TRIGGER_SEVERITY) == ITEM_DATA_NOT_NULL)
